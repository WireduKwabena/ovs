...
export const SystemAdminLayout: React.FC<SystemAdminLayoutProps> = ({ children }) => {
  ...
  const navItems = [
    { to: getPlatformAdminPath('dashboard'), label: 'Platform Dashboard', icon: LayoutDashboard },
    { to: '/admin/platform/ai-engine', label: 'AI Infrastructure', icon: Brain },
    { to: '/admin/platform/registry', label: 'Organization Registry', icon: Building2 },
    { to: '/admin/platform/billing', label: 'Billing & Plans', icon: CreditCard },
    { to: '/admin/platform/health', label: 'System Health', icon: Activity },
    { to: '/admin/platform/logs', label: 'Platform Audit Logs', icon: FileSearch },
  ];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-border/70 bg-background/95 px-4 backdrop-blur-xl lg:hidden">
        <Link to={getPlatformAdminPath('dashboard')} className="flex items-center gap-2">
        ...
        <aside className={[
          ...
          <div className="flex h-full flex-col p-4">
            ...
            <div className="mb-8 hidden px-2 lg:block">
              <Link to={getPlatformAdminPath('dashboard')} className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">System Admin</p>
            ...
            <div className="mb-6 rounded-2xl border border-border/70 bg-card/50 p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[11px] text-muted-foreground uppercase tracking-wider">Superuser Access</p>
                </div>
              </div>
            </div>
            ...
            <nav className="flex-1 space-y-1">
              <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Main Platform</p>
              {navItems.map((item) => {
