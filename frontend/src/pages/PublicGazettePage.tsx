...
export const PublicGazettePage: React.FC = () => {
  ...
  const loadFeed = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        // Surface the primary error — it's more informative than the fallback error
        setError(primaryErr instanceof Error ? primaryErr.message : "Unable to load gazette feed.");
      }
    }
  }, [appliedSearch, appliedStatus]);
  ...
  const gazetteNumberCount = useMemo(
    () => records.filter((item) => Boolean(item.gazette_number)).length,
    [records],
  );
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border bg-background">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-primary">Public Record</p>
            <h1 className="text-2xl font-bold text-foreground">Government Gazette Feed</h1>
            <p className="text-sm text-muted-foreground">Published appointments only. Internal vetting data is not displayed.</p>
          </div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <form onSubmit={handleApplyFilters} className="mb-6 rounded-xl border border-border bg-card p-4 shadow-sm">
          <div className="grid gap-3 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_auto_auto]">
            ...
            <button
              ...
              className="h-10 rounded-md border border-border bg-transparent px-4 text-sm font-semibold text-foreground transition hover:bg-muted"
            >
              Clear
            </button>
          </div>
        ...
        <section className="mb-6 grid gap-4 md:grid-cols-2">
          <article className="rounded-xl border border-border bg-card p-4">
            ...
            <p className="text-sm text-muted-foreground">Gazette entries</p>
            <p className="text-2xl font-bold text-foreground">{records.length}</p>
          </article>
          <article className="rounded-xl border border-border bg-card p-4">
            <div className="mb-2 inline-flex rounded-md bg-muted p-2 text-muted-foreground">
              <FileText className="h-4 w-4" />
            </div>
            <p className="text-sm text-muted-foreground">With gazette number</p>
            <p className="text-2xl font-bold text-foreground">{gazetteNumberCount}</p>
          </article>
        </section>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              {hasFilters ? (
                <button
                  type="button"
                  onClick={handleClearFilters}
                  className="inline-flex rounded-md border border-rose-300 bg-white px-3 py-2 text-sm font-semibold text-rose-900 hover:bg-rose-100"
                >
            ...
            {records.map((record) => (
              <article key={record.id} className="rounded-xl border border-border bg-card p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h2 className="text-lg font-semibold text-foreground">{record.position_title}</h2>
                ...
                <div className="mt-3 grid gap-2 text-sm text-muted-foreground md:grid-cols-2">
                  ...
                  <p>
                    <span className="font-semibold text-foreground">Gazette number:</span>{" "}
                    {record.gazette_number || "Pending"}
                  </p>
                  <p>
                    <span className="font-semibold text-foreground">Gazette date:</span>{" "}
                    {formatPublicDate(record.gazette_date)}
                  </p>
                  <p className="md:col-span-2">
                    <span className="font-semibold text-foreground">Publication reference:</span>{" "}
                    {record.publication_reference || "Not provided"}
                  </p>
                  <p className="md:col-span-2">
                    <span className="font-semibold text-foreground">Published at:</span>{" "}
