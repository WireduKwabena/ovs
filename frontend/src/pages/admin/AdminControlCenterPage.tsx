        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
          {ADMIN_SECTIONS.map((section) => (
            <article key={section.path} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-1 text-xs text-slate-600">Direct model administration panel.</p>
              <a
                href={`${adminBaseUrl}${section.path}`}
                target="_blank"
                rel="noreferrer"
                className="mt-3 inline-flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100"
              >
                Open
