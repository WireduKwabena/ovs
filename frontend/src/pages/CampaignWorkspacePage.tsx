...
const getSourceRubricId = (
  ...
  return typeof sourceRubricId === "string" && sourceRubricId.length > 0
    ? sourceRubricId
    : null;
};
...
const CampaignWorkspacePage: React.FC = () => {
  ...
  const [campaignRubricVersions, setCampaignRubricVersions] = useState<
    CampaignRubricVersion[]
  >([]);
  const [selectedRubricId, setSelectedRubricId] = useState<string>("");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [applyingRubric, setApplyingRubric] = useState(false);
  const [activatingRubricVersionId, setActivatingRubricVersionId] = useState<
    string | null
  >(null);
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    try {
      ...
      const activeVersion = rubricVersionsData.find(
        (version) => version.is_active,
      );
      const sourceRubricId =
        getSourceRubricId(activeVersion?.rubric_payload) ?? "";
      if (sourceRubricId) {
        setSelectedRubricId(sourceRubricId);
      } else if (rubricsData.length > 0) {
        setSelectedRubricId(
          (previous) => previous || String(rubricsData[0].id),
        );
      } else {
        setSelectedRubricId("");
      }
      ...
      setError(
        getErrorMessage(err, "Failed to load appointment exercise workspace."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const linkedOfficeIds = useMemo(() => {
    ...
    if (
      Array.isArray(campaign.position_ids) &&
      campaign.position_ids.length > 0
    ) {
  ...
  const selectedRubric = useMemo(
    () =>
      availableRubrics.find(
        (rubric) => String(rubric.id) === selectedRubricId,
      ) ?? null,
    ...
    try {
      ...
      if (quotaImportError) {
        ...
        if (quotaImportError.quotaPatch) {
          setQuota((current) =>
            current ? { ...current, ...quotaImportError.quotaPatch } : current,
          );
        }
      } else {
  ...
  const handleApplyRubricVersion = async () => {
    ...
    try {
      ...
      const weightInterview =
        selectedRubric.interview_weight + selectedRubric.manual_review_weight;
    ...
    <main className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-6 xl:px-8">
      <section className="rounded-2xl bg-white border border-slate-200 p-6">
        <div className="flex flex-wrap gap-3 items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-semibold mt-2">
              Appointment Exercise: {campaign.name}
            </h1>
        ...
        <div className="mt-4 text-sm text-slate-700">
          Status:{" "}
          <span className="font-medium text-slate-700">{campaign.status}</span>{" "}
      ...
      <section className="rounded-xl border border-cyan-200 bg-cyan-50 p-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-cyan-900">
          Office-Centered Progress
        </p>
        <p className="mt-2 text-sm text-cyan-900">
          This appointment exercise is linked to {linkedOfficeIds.length} office
          {linkedOfficeIds.length === 1 ? "" : "s"}. Continue from intake to
          nomination file review and publication.
        </p>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="text-sm text-slate-700 mb-4">
              Select an existing rubric and snapshot it into this appointment
              exercise as a new rubric version.
            </p>
            <div className="mb-4 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs text-indigo-900">
              Hover or focus the{" "}
              <Info className="mx-1 inline h-3.5 w-3.5 align-text-bottom" />{" "}
              icons beside labels for guidance.
            </div>
              ...
              {availableRubrics.length === 0 ? (
                ...
                availableRubrics.map((rubric) => (
                  <option key={rubric.id} value={String(rubric.id)}>
                    ...
                    {rubric.status ??
                      (rubric.is_active ? "active" : "archived")}
                    )
                  </option>
                ))
              )}
            ...
            {selectedRubric ? (
              <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-800">
                ...
                <p className="text-xs text-slate-700 mt-1">
                  Pass: {selectedRubric.passing_score}% | Auto-approve:{" "}
                  {selectedRubric.auto_approve_threshold}% | Auto-reject:{" "}
                  {selectedRubric.auto_reject_threshold}%
                </p>
                <p className="text-xs text-slate-700 mt-1">
                  Criteria: {selectedRubric.criteria?.length ?? 0} | Type:{" "}
                  {selectedRubric.rubric_type}
                </p>
              </div>
            ) : null}
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            {campaignRubricVersions.length === 0 ? (
              <p className="text-sm text-slate-700">
                No exercise rubric versions yet.
              </p>
            ) : (
              <div className="space-y-3 max-h-80 overflow-auto pr-1">
                {campaignRubricVersions.map((version) => (
                  ...
                  >
                    {(() => {
                      const sourceRubricId = getSourceRubricId(
                        version.rubric_payload,
                      );
                      return (
                        <>
                          ...
                          <p className="mt-1 text-xs text-slate-700">
                            Doc Weight: {version.weight_document}% | Interview
                            Weight: {version.weight_interview}% | Pass:{" "}
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-2">
                            {!version.is_active && (
                              <button
                                type="button"
                                onClick={() =>
                                  void handleActivateRubricVersion(version.id)
                                }
                                disabled={
                                  activatingRubricVersionId === version.id
                                }
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {canManageRegistry ? (
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            {shouldShowQuota && (
              <div className="mb-4 rounded-lg border border-cyan-200 bg-cyan-50 p-3 text-sm text-cyan-900">
                ...
                {quotaError && (
                  <p className="mt-1 text-xs text-red-700">{quotaError}</p>
                )}
                ...
                {!quotaLoading && quota && (
                  <div className="mt-2 space-y-1 text-xs text-cyan-900">
                    ...
                    <p>
                      Window: {formatDate(quota.period_start)} -{" "}
                      {formatDate(quota.period_end)}
                    </p>
                    <p>
                      Payload estimate:{" "}
                      <span className="font-semibold">
                        {importProjection.count}
                      </span>{" "}
                      unique nominee intake rows
                      {quota.limit !== null && (
                        <>
                          {" "}
                          | Projected usage:{" "}
                    ...
                    {projectedQuotaExceeded &&
                      quota &&
                      quota.limit !== null && (
                        <p className="font-medium text-red-700">
                          Projected import exceeds quota ({projectedUsage}/
                          {quota.limit}). Reduce payload size or upgrade plan
                          before importing.
                        </p>
                      )}
                    <p className="text-slate-700">
                      Estimate may be lower after import if payload rows are
                      duplicates or already enrolled.
                    </p>
                  </div>
                )}
              </div>
            ...
            <form className="space-y-3" onSubmit={handleImportCandidates}>
              ...
              <div className="inline-flex items-center gap-2 text-sm text-slate-700">
                ...
                />
                <label htmlFor="campaign-send-invites">
              ...
              <button
                ...
                disabled={
                  importing ||
                  Boolean(importProjection.parseError) ||
                  projectedQuotaExceeded
                }
            ...
            {importSummary && (
              <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">
                <p>
                  Imported: {importSummary.created_candidates} new nominee
                  intake profiles, {importSummary.created_enrollments}{" "}
          ...
          <div className="rounded-xl border border-slate-100 bg-slate-50 p-5 flex items-center justify-center">
            <p className="text-sm text-slate-500">
              Nominee pool import is managed by registry administrators.
            </p>
          </div>
        )}
                  ...
                  <div>
                    <p className="font-medium text-slate-800">
                      {enrollment.candidate_email ||
                        `Profile #${enrollment.candidate}`}
                    </p>
                    <p className="text-xs text-slate-700">
                      Invited:{" "}
                      {enrollment.invited_at
                        ? formatDateTime(enrollment.invited_at)
                        : "N/A"}
                    </p>
                  </div>
                  ...
                  {canManageRegistry &&
                    ![
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          void handleMarkEnrollmentComplete(enrollment.id)
                        }
                      ...
                      >
                        ...
                        {completingEnrollmentId === enrollment.id
                          ? "Updating..."
                          : "Mark Complete"}
                      </button>
                    )}
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        {latestInvitations.length === 0 ? (
          <p className="text-slate-700 text-sm">
            No invitations found for this appointment exercise.
          </p>
        ) : (
          <>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-700">
                        {invitation.channel.toUpperCase()} • Attempts:{" "}
                        {invitation.attempts}
                      </p>
                    </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {invitation.accept_url && (
                            <button
                              type="button"
                              onClick={() =>
                                void handleCopyAcceptUrl(invitation)
                              }
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <p className="text-sm text-slate-700">
          Nominees should use the access URL received through email/SMS. Legacy
          invitation links are still accepted at{" "}
          <code className="bg-slate-200 px-1 py-0.5 rounded text-xs text-slate-900">
            /invite/&lt;token&gt;
          </code>
          .
        </p>
