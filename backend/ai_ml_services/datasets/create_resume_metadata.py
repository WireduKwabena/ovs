"""Build normalized metadata for resume datasets (PDF-first)."""

from __future__ import annotations

import argparse
import logging
import random
import re
from pathlib import Path
from typing import Dict, Iterable, List, Sequence, Tuple

import pandas as pd

logger = logging.getLogger(__name__)

DEFAULT_EXTENSIONS = {".pdf"}

# Keyed by compact tokens (lowercase, non-alnum removed).
RESUME_LABEL_ALIASES: Dict[str, str] = {
    "accountant": "accountant",
    "advocate": "advocate",
    "agricultural": "agricultural",
    "agriculture": "agriculture",
    "apparel": "apparel",
    "architect": "architect",
    "architects": "architect",
    "arts": "arts",
    "automobile": "automobile",
    "aviation": "aviation",
    "banking": "banking",
    "blockchain": "blockchain",
    "bpo": "bpo",
    "pbo": "bpo",
    "buildingconstruction": "building construction",
    "businessanalyst": "business analyst",
    "civilengineer": "civil engineer",
    "consult": "consultant",
    "consultant": "consultant",
    "datascience": "data science",
    "database": "database",
    "design": "design",
    "designer": "designer",
    "designing": "design",
    "devopsengineer": "devops engineer",
    "digital": "digital",
    "digitalmedia": "digital media",
    "dot": "dotnet developer",
    "dotnetdeveloper": "dotnet developer",
    "education": "education",
    "electricalengineer": "electrical engineer",
    "etl": "etl developer",
    "etldeveloper": "etl developer",
    "finance": "finance",
    "food": "food and beverages",
    "foodbeverages": "food and beverages",
    "healthfitness": "health fitness",
    "hr": "human resources",
    "humanresources": "human resources",
    "informationtechnology": "information technology",
    "it": "information technology",
    "javadeveloper": "java developer",
    "management": "management",
    "managment": "management",
    "mechanicalengineer": "mechanical engineer",
    "networksecurityengineer": "network security engineer",
    "nse": "network security engineer",
    "operationmanager": "operations manager",
    "operationsmanager": "operations manager",
    "pmo": "pmo",
    "public": "public relations",
    "publicrelations": "public relations",
    "pythondeveloper": "python developer",
    "react": "react developer",
    "reactdeveloper": "react developer",
    "sales": "sales",
    "sapdeveloper": "sap developer",
    "sql": "sql developer",
    "sqldeveloper": "sql developer",
    "testing": "testing",
    "webdesigning": "web designing",
}


def _compact_key(text: str) -> str:
    return re.sub(r"[^a-z0-9]+", "", text.lower())


def normalize_resume_label(raw_label: str) -> str:
    label = raw_label.lower().strip()
    label = label.replace("_", " ")
    label = re.sub(r"\s+resumes?$", "", label).strip()
    label = re.sub(r"\s+", " ", label)
    key = _compact_key(label)
    canonical = RESUME_LABEL_ALIASES.get(key)
    if canonical:
        return canonical
    return label


def iter_resume_files(root_dir: Path, extensions: Sequence[str]) -> Iterable[Tuple[Path, str]]:
    allowed = {ext.lower() if ext.startswith(".") else f".{ext.lower()}" for ext in extensions}
    for class_dir in sorted(path for path in root_dir.iterdir() if path.is_dir()):
        for file_path in class_dir.rglob("*"):
            if file_path.is_file() and file_path.suffix.lower() in allowed:
                yield file_path, class_dir.name


def _assign_stratified_splits(
    df: pd.DataFrame,
    val_ratio: float,
    test_ratio: float,
    random_seed: int,
) -> pd.DataFrame:
    result = df.copy()
    result["split"] = "train"
    rng = random.Random(random_seed)

    for label in sorted(result["label"].unique()):
        label_indices = result.index[result["label"] == label].tolist()
        if len(label_indices) < 2:
            continue
        rng.shuffle(label_indices)

        val_count = int(round(len(label_indices) * val_ratio))
        test_count = int(round(len(label_indices) * test_ratio))
        while val_count + test_count >= len(label_indices):
            if test_count > 0:
                test_count -= 1
            elif val_count > 0:
                val_count -= 1
            else:
                break

        val_indices = label_indices[:val_count]
        test_indices = label_indices[val_count : val_count + test_count]
        if val_indices:
            result.loc[val_indices, "split"] = "val"
        if test_indices:
            result.loc[test_indices, "split"] = "test"
    return result


def build_resume_metadata(
    source_dir: Path,
    output_dir: Path,
    val_ratio: float = 0.15,
    test_ratio: float = 0.15,
    random_seed: int = 42,
    min_samples_per_label: int = 1,
    extensions: Sequence[str] = (".pdf",),
) -> pd.DataFrame:
    if val_ratio < 0 or test_ratio < 0:
        raise ValueError("val_ratio and test_ratio must be >= 0.")
    if val_ratio + test_ratio >= 1:
        raise ValueError("val_ratio + test_ratio must be < 1.")
    if min_samples_per_label < 1:
        raise ValueError("min_samples_per_label must be >= 1.")

    rows: List[Dict[str, str]] = []
    for file_path, raw_label in iter_resume_files(source_dir, extensions):
        normalized_label = normalize_resume_label(raw_label)
        rows.append(
            {
                "filename": file_path.name,
                "filepath": str(file_path.resolve()),
                "label_raw": raw_label,
                "label": normalized_label,
            }
        )

    output_dir.mkdir(parents=True, exist_ok=True)
    metadata_path = output_dir / "metadata.csv"
    labels_path = output_dir / "labels.csv"
    aliases_path = output_dir / "raw_to_normalized_labels.csv"

    df = pd.DataFrame(rows)
    if df.empty:
        df.to_csv(metadata_path, index=False)
        pd.DataFrame(columns=["label_id", "label"]).to_csv(labels_path, index=False)
        pd.DataFrame(columns=["label_raw", "label"]).to_csv(aliases_path, index=False)
        logger.warning("No resume files found. Wrote empty metadata to %s", metadata_path)
        return df

    if min_samples_per_label > 1:
        counts = df["label"].value_counts()
        keep = set(counts[counts >= min_samples_per_label].index.tolist())
        removed = len(df) - int(df["label"].isin(keep).sum())
        df = df[df["label"].isin(keep)].copy()
        if removed > 0:
            logger.info("Removed %d samples from labels below min_samples_per_label.", removed)

    if df.empty:
        df.to_csv(metadata_path, index=False)
        pd.DataFrame(columns=["label_id", "label"]).to_csv(labels_path, index=False)
        pd.DataFrame(columns=["label_raw", "label"]).to_csv(aliases_path, index=False)
        logger.warning("No resume files left after filtering. Wrote empty metadata to %s", metadata_path)
        return df

    labels = sorted(df["label"].unique().tolist())
    label_to_id = {label: idx for idx, label in enumerate(labels)}
    df["label_id"] = df["label"].map(label_to_id)

    df = _assign_stratified_splits(
        df,
        val_ratio=val_ratio,
        test_ratio=test_ratio,
        random_seed=random_seed,
    )
    df.to_csv(metadata_path, index=False)

    pd.DataFrame(
        [{"label_id": idx, "label": label} for label, idx in label_to_id.items()]
    ).sort_values("label_id").to_csv(labels_path, index=False)

    (
        df[["label_raw", "label"]]
        .drop_duplicates()
        .sort_values(["label", "label_raw"])
        .to_csv(aliases_path, index=False)
    )

    logger.info(
        "Resume metadata written: %s (total=%d train=%d val=%d test=%d labels=%d)",
        metadata_path,
        len(df),
        int((df["split"] == "train").sum()),
        int((df["split"] == "val").sum()),
        int((df["split"] == "test").sum()),
        len(labels),
    )
    return df


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build normalized metadata for resume datasets."
    )
    parser.add_argument(
        "--source_dir",
        type=str,
        default="ai_ml_services/datasets/raw_dataset/Resumes PDF",
        help="Root directory containing per-class resume folders.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="ai_ml_services/datasets/processed/resumes",
        help="Directory where metadata files are written.",
    )
    parser.add_argument(
        "--val_ratio",
        type=float,
        default=0.15,
        help="Validation split ratio per label.",
    )
    parser.add_argument(
        "--test_ratio",
        type=float,
        default=0.15,
        help="Test split ratio per label.",
    )
    parser.add_argument(
        "--min_samples_per_label",
        type=int,
        default=1,
        help="Drop labels with fewer samples than this threshold.",
    )
    parser.add_argument(
        "--random_seed",
        type=int,
        default=42,
        help="Random seed used for split assignment.",
    )
    parser.add_argument(
        "--extensions",
        nargs="*",
        default=sorted(DEFAULT_EXTENSIONS),
        help="Allowed file extensions (example: .pdf .docx).",
    )
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
    build_resume_metadata(
        source_dir=Path(args.source_dir),
        output_dir=Path(args.output_dir),
        val_ratio=args.val_ratio,
        test_ratio=args.test_ratio,
        random_seed=args.random_seed,
        min_samples_per_label=args.min_samples_per_label,
        extensions=args.extensions,
    )


if __name__ == "__main__":
    main()
