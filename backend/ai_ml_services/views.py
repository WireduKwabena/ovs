...
# ---------------------------------------------------------------------------
# Tavus callback & custom LLM endpoint
# ---------------------------------------------------------------------------

from rest_framework.decorators import api_view, permission_classes as _permission_classes
...
class TavusLLMRequestView(APIView):
    ...
    Tavus sends the conversation so far (system + user messages) and expects
    back the next interviewer turn generated by Claude.

    Endpoint: POST /api/v1/ai-monitor/tavus/llm/
    Security: Verified via SERVICE_TOKEN header set in Tavus conversation config.
    """
    ...
    def post(self, request):
        ...
        # Basic service-token check (Tavus sends this as Bearer or custom header)
        expected_token = str(getattr(settings, "SERVICE_TOKEN", "")).strip()
        if expected_token:
        ...
        try:
            reply = handle_tavus_llm_request(messages)
        except RuntimeError as exc:
            return Response({"error": str(exc)}, status=status.HTTP_503_SERVICE_UNAVAILABLE)

        # Return OpenAI-compatible response format that Tavus expects
        return Response(
            {
                "choices": [
                    {
...
class TavusEventCallbackView(APIView):
    ...
    ended, recording ready, etc.).

    Endpoint: POST /api/v1/ai-monitor/tavus/events/
    """
    ...
    def post(self, request):
        ...
        _log.info("Tavus event received: type=%s conversation=%s", event_type, conversation_id)

        if event_type == "conversation.ended" and conversation_id:
            # Trigger any cleanup async tasks here if needed
            pass

        return Response({"status": "received"}, status=status.HTTP_200_OK)


tavus_llm_request_view = TavusLLMRequestView.as_view()
tavus_event_callback_view = TavusEventCallbackView.as_view()
