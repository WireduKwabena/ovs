...
class SocialProfileCheckResultSerializer(serializers.ModelSerializer):
    """Serializer for social profile check results."""

    application_case_id = serializers.CharField(source="application.case_id", read_only=True)
    risk_level_display = serializers.CharField(source="get_risk_level_display", read_only=True)

    class Meta:
        model = SocialProfileCheckResult
        fields = [
            "id",
            "application",
            "application_case_id",
        ...
        read_only_fields = ["id", "checked_at", "updated_at"]
