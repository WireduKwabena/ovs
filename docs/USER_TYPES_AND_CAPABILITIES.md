...
### Active organization and tenant-resolution headers

Two headers are operationally important in multi-organization mode:

- `X-Organization-Slug`: used for tenant resolution into the correct organization schema context.
- `X-Active-Organization-ID`: used for org-scoped authorization checks and policy evaluation when users have multiple memberships.

Operational guidance:

- Missing or invalid active organization context should be treated as scope failure for org-bound operations.
- Header context does not bypass capability checks; both capability and scope must pass.
